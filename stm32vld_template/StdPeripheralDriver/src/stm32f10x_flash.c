...
void FLASH_HalfCycleAccessCmd(uint32_t FLASH_HalfCycleAccess) {
	...
	/* Enable or disable the Half cycle access */FLASH ->ACR &= ACR_HLFCYA_Mask;
...
void FLASH_PrefetchBufferCmd(uint32_t FLASH_PrefetchBuffer) {
	...
	/* Enable or disable the Prefetch Buffer */FLASH ->ACR &= ACR_PRFTBE_Mask;
...
FLASH_Status FLASH_EraseOptionBytes(void) {
	...
	if (status == FLASH_COMPLETE) {
		...
		/* if the previous operation is completed, proceed to erase the option bytes */FLASH ->CR |=
				CR_OPTER_Set;
		FLASH ->CR |= CR_STRT_Set;
		/* Wait for last operation to be completed */
		status = FLASH_WaitForLastOperation(EraseTimeout );
		...
		if (status == FLASH_COMPLETE) {
			...
			/* Enable the Option Bytes Programming operation */FLASH ->CR |=
					CR_OPTPG_Set;
			/* Restore the last read protection Option Byte value */OB ->RDP =
					(uint16_t) rdptmp;
			/* Wait for last operation to be completed */
...
FLASH_Status FLASH_ProgramOptionByteData(uint32_t Address, uint8_t Data) {
	...
	if (status == FLASH_COMPLETE) {
		...
		FLASH ->OPTKEYR = FLASH_KEY1;
		FLASH ->OPTKEYR = FLASH_KEY2;
		/* Enables the Option Bytes Programming operation */FLASH ->CR |=
				CR_OPTPG_Set;
		*(__IO uint16_t*) Address = Data;
...
FLASH_Status FLASH_UserOptionByteConfig(uint16_t OB_IWDG, uint16_t OB_STOP,
	...
	/* Check the parameters */
	assert_param(IS_OB_IWDG_SOURCE(OB_IWDG)); assert_param(IS_OB_STOP_SOURCE(OB_STOP)); assert_param(IS_OB_STDBY_SOURCE(OB_STDBY));

	/* Authorize the small information block programming */FLASH ->OPTKEYR =
			FLASH_KEY1;
	FLASH ->OPTKEYR = FLASH_KEY2;
...
#else
	/* Check the parameters */
	assert_param(IS_FLASH_IT(FLASH_IT)); assert_param(IS_FUNCTIONAL_STATE(NewState));
